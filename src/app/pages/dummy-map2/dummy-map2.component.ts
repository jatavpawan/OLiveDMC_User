...
declare var $: any;
@Component({
  selector: 'app-dummy-map2',
  templateUrl: './dummy-map2.component.html',
  styleUrls: ['./dummy-map2.component.css']
})
export class DummyMap2Component implements OnInit {
...
Highcharts: typeof Highcharts = Highcharts;
  ...
  constructor() {
    this.searchText = 'Search ' + this.mapCount + ' maps';
    this.mapOptions = '<option value="custom/world.js">' + this.searchText + '</option>' + this.mapOptions;
...
$.each(Highcharts.maps, function (mapGroup, maps) {
  if (mapGroup !== "version") {
      this.mapOptions += '<option class="option-header">' + mapGroup + '</option>';
      $.each(maps, function (desc, path) {
          this.mapOptions += '<option value="' + path + '">' + desc + '</option>';
          this.mapCount += 1;
      });
  }
  });

$("#mapDropdown").append(this.mapOptions).combobox();
...
var $selectedItem = $("option:selected", this),
    ...
    svgPath = this.baseMapPath + mapKey + '.svg',
    geojsonPath = this.baseMapPath + mapKey + '.geo.json',
    javascriptPath = this.baseMapPath + this.value,
    isHeader = $selectedItem.hasClass('option-header');

// Dim or highlight search box
if (mapDesc === this.searchText || isHeader) {
...
function mapReady() {
    ...
    this.showDataLabels = $("#chkDataLabels").prop('checked');
