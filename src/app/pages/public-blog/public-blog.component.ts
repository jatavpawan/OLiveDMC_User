...
export class PublicBlogComponent implements OnInit {
  ...
  // searchBlogger = new FormControl("");
  searchBlogList: Observable<any[]>;
  ...
  constructor(
    ...
    if (this.authService.isLoggedIn()) {
      ...
      this.blogForm
        .get("userName")
        .setValue(
          this.loggedInUserData.firstName + " " + this.loggedInUserData.lastName
        );
  ...
  ngOnInit(): void {
    this.searchBlogList = this.searchBlogForm.get("searchBlogger").valueChanges.pipe(
      startWith(""),
      debounceTime(400),
      switchMap((val) => {
        debugger;
        if (val.length >= 1) {
          return this.searchBlogByText(val);
        } else {
          return [];
        }
      })
    ...
    this.tinymceConfig.images_upload_handler = function (
      blobInfo,
      success,
      failure
    ) {
  ...
  searchBlogByText(val: string): Observable<any[]> {
    debugger;
    let obj = {
      PageNo: 0,
      text: val,
    ...
    return this.blogService.searchBlogByText(obj).pipe(
      map((response) => {
  ...
  submitBlogData() {
    ...
    if (this.blogForm.valid && this.blogForm.get("category").value != "0") {
      ...
      formData.append(
        "ShortDescription",
        this.blogForm.get("shortDescription").value
      );
      formData.append("Status", this.blogForm.get("status").value);
      formData.append(
        "ApprovalStatus",
        this.blogForm.get("approvalStatus").value
      );
  ...
  shortDescriptionCharacterCount(event): boolean {
    if (
      this.blogForm.get("shortDescription").value.length >= 200 &&
      event.keyCode != 8
    ) {
      let shortDescription: string = this.blogForm.get("shortDescription")
        .value;
      this.blogForm
        .get("shortDescription")
        .setValue(shortDescription.substring(0, 200));
  ...
  resetSearchBlog() {
    this.searchBlogForm.get('searchBlogger').setValue("")
    this.allCategoryBlog();
  }

  displayBlogProperty(value) {
    if (value) {
      return value.title;
    }
  }
